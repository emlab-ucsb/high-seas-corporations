...
# Add Corporate Actors

```{r}
vessels_corp_data <- readxl::read_xlsx("data/HS_vessels_info_100818.xlsx") %>% 
  janitor::clean_names() %>% 
  select(ssvid = mmsi, owner, owner_source, corporate_actor, corporate_actor_source, main_company, main_company_flag, subsidiaries_and_affiliates, main_company_and_subsidiaries_source, remarks)

reefers_corp_data <- readxl::read_xlsx("data/HS_reefers_and_bunkers_info_100818.xlsx") %>% 
  janitor::clean_names() %>% 
  select(ssvid, owner, owner_source, corporate_actor, corporate_actor_source, main_company, main_company_flag, subsidiaries_and_affiliates, main_company_and_subsidiaries_source, remarks)
```
...
HS_vessels %>% 
  summarize(HS_effort = sum(fishing_hours_HS),
            HS_effort_with_corp_actor = sum(fishing_hours_HS[!is.na(corporate_actor)])/HS_effort,
            HS_effort_with_owner = sum(fishing_hours_HS[!is.na(corporate_actor) | !is.na(gfw_owner)])/HS_effort,
            HS_effort_with_no_idea = sum(fishing_hours_HS[is.na(corporate_actor) & is.na(gfw_owner)])/HS_effort,
            )
```
...
write_csv(HS_reefers_and_bunkers, "saved_files/HS_reefers_and_bunkers_101019.csv")

write_csv(HS_reefers_and_bunkers, file.path(DIR_drive, "results/HS_reefers_and_bunkers_101019.csv"))

write_csv(HS_vessels, "saved_files/HS_vessels_101019.csv")
write_csv(HS_vessels, file.path(DIR_drive, "results/HS_vessels_101019.csv"))
...
write_csv(high_seas_encounters, "saved_files/HS_encounters_101019.csv")
write_csv(high_seas_encounters, file.path(DIR_drive, "results/HS_encounters_101019.csv"))
```

# Map High Seas fishing effort
...
GROUP BY
  ...
  lon_bin,
  ssvid",
 ssvid_of_interest = pull(HS_vessels, ssvid),
  .con = BQ_connection)
...
HS_binned_effort %>% 
  ...
  summarize(h = sum(fishing_hours, na.rm = T)) %>% 
  filter(h > .1) %>% 
  ggplot()+
  geom_raster(aes(x = lon_bin, y = lat_bin, fill = h))
```

```{r}
write_csv(HS_binned_effort, "saved_files/HS_binned_effort_100919.csv")
write_csv(HS_binned_effort, file.path(DIR_drive, "results/HS_binned_effort_100919.csv"))
```
